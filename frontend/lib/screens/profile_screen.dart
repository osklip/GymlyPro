...
// POPRAWKA: Ukrycie klasy AuthProvider z pakietu Firebase w celu wyeliminowania kolizji nazw
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Widget _buildWeightChart(List<BodyMeasurement> measurements) {
    ...
    final ascList =
        List<BodyMeasurement>.from(measurements).reversed.toList();
    ...
    return Padding(
      padding: const EdgeInsets.only(
          right: 16.0, left: 4.0, top: 16.0, bottom: 8.0),
      child: SizedBox(
        height: 200,
        child: LineChart(
          LineChartData(
            ...
            children: [
              ...
              TextFormField(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'Nowe hasło (pozostaw puste, by nie zmieniać)')),
                    ...
                    : () async {
                        ...
                        try {
                          if (newName.isNotEmpty && newName != currentName) {
                            ...
                            await http.patch(
                                Uri.parse(
                                    'http://10.0.2.2:8000/users/me?display_name=$newName'),
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                              content: Text('Konto zaktualizowane pomyślnie.'),
  ...
  void _showAddMeasurementDialog(BuildContext context) {
    ...
    if (measurementProvider.measurements.isNotEmpty) {
      heightController.text =
          measurementProvider.measurements.first.height.toString();
    }
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'Tkanka tłuszczowa % (Opcjonalnie)'),
                  keyboardType:
                      const TextInputType.numberWithOptions(decimal: true)),
              const SizedBox(height: 24),
              ElevatedButton(
                  ...
                  onPressed: () async {
                    if (formKey.currentState?.validate() ?? false) {
                      final w = double.parse(
                          weightController.text.replaceAll(',', '.'));
                      final h = double.parse(
                          heightController.text.replaceAll(',', '.'));
                      final bfText = bfController.text.trim();
                      final bf = bfText.isNotEmpty
                          ? double.parse(bfText.replaceAll(',', '.'))
                          : null;
                      try {
                        await measurementProvider.addMeasurement(
                            BodyMeasurement(
                                weight: w,
                                height: h,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Profil & Analityka',
      ...
      body: _buildBody(
          context, userProvider, measurementProvider, achievementProvider),
    );
  }

  Widget _buildBody(BuildContext context, UserProvider uProvider,
      MeasurementProvider mProvider, AchievementProvider aProvider) {
        ...
        children: [
          if (profile != null) ...[
            ...
            TextButton.icon(
              ...
              onPressed: () =>
                  _showEditProfileModal(context, profile.displayName),
            ),
            const SizedBox(height: 12),
            Card(
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Container(
                        height: 40, width: 1, color: const Color(0xFF334155)),
                    Column(children: [
                      const Text('PUNKTY EXP',
                          style: TextStyle(
                              color: Color(0xFF94A3B8),
                              fontSize: 11,
                              fontWeight: FontWeight.bold)),
            ...
            ElevatedButton.icon(
              ...
              label: const Text('GLOBALNY RANKING LIDERÓW',
                  style: TextStyle(
                      fontWeight: FontWeight.w900,
                      color: Colors.white,
                      letterSpacing: 0.5)),
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (mProvider.isLoading && mProvider.measurements.isEmpty)
                    ...
                    _buildWeightChart(mProvider.measurements),
                ],
              ),
            ),
          ),
          const SizedBox(height: 24),
          const Text('Gablota Osiągnięć',
              style: TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.w900,
                  color: Color(0xFF64748B))),
          ...
          if (aProvider.isLoading && aProvider.allAchievements.isEmpty)
            ...
            ...aProvider.allAchievements.map((ach) {
              ...
              return Padding(
                ...
                child: Card(
                  color:
                      isUnlocked ? const Color(0xFFECFDF5) : Colors.white,
                  shape: RoundedRectangleBorder(
                  ...
                  child: ListTile(
                    ...
                    leading: CircleAvatar(
                        ...
                        child: Icon(_getIcon(ach.iconUrl),
                            color: isUnlocked
                                ? Colors.white
                        ...
                        children: [
                          ...
                          if (isUnlocked && earnedDate != null)
                            ...
                            Padding(
                                padding: const EdgeInsets.only(top: 4.0),
                                child: Text(
                                    'Wymagane: ${ach.requiredPoints} pkt',
                                    style: const TextStyle(
                                        color: Color(0xFF94A3B8),
                                        fontSize: 11,
                                        fontWeight: FontWeight.bold)))
          ...
          const SizedBox(height: 24),
          const Text('Historia wpisów wagowych',
              style: TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.w900,
                  color: Color(0xFF64748B))),
          ...
          ...mProvider.measurements.map((m) => Card(
              ...
              child: ListTile(
                  dense: true,
                  leading: const Icon(Icons.scale, color: Color(0xFF10B981)),
                  title: Text('${m.weight} kg',
                      style: const TextStyle(
                          ...
                          color: Color(0xFF0F172A))),
                  subtitle: Text(
                      'Wzrost: ${m.height} cm ${m.bodyFatPercentage != null ? '| BF: ${m.bodyFatPercentage}%' : ''}',
                      style: const TextStyle(color: Color(0xFF64748B))),
                  trailing: m.measuredAt != null
                      ? Text(
                          '${m.measuredAt!.day.toString().padLeft(2, '0')}.${m.measuredAt!.month.toString().padLeft(2, '0')}',
                          style: const TextStyle(
                              fontWeight: FontWeight.bold,
                              color: Color(0xFF94A3B8)))
                      : null))),
