  ...
  Future<WorkoutSession> startSession(int? planId) async {
    try {
      ...
      if (response.statusCode == 201) {
        ...
        throw Exception('Błąd podczas uruchamiania sesji treningowej (Kod: ${response.statusCode}).');
      }
    } on SocketException {
  ...
  Future<WorkoutSet> addSet(int sessionId, WorkoutSet workoutSet) async {
    try {
      ...
      if (response.statusCode == 201) {
        ...
        throw Exception('Błąd podczas zapisu serii (Kod: ${response.statusCode}).');
      }
    } on SocketException {
  ...
  Future<WorkoutSession> finishSession(int sessionId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Nie udało się poprawnie zakończyć i przeliczyć treningu (Kod: ${response.statusCode}).');
      }
    } on SocketException {
  ...
  Future<List<WorkoutSession>> getHistory() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Błąd pobierania historii treningów (Kod: ${response.statusCode}).');
      }
    } on SocketException {
      throw Exception('Brak sieci. Historia treningów jest niedostępna w trybie offline.');
    }
  }
}